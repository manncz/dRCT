...
loop_glmnet <- function(Y,Tr,Z,offset=rep(0,length(Y)),...){
    require(glmnet)
    N = length(Y)
    dat = data.frame(Y,Tr, Z)

    fullModT <- cv.glmnet(Y~.-Tr,data=dat,subset=Tr==1,offset=offset,...)
    fullModC <- cv.glmnet(Y~.-Tr,data=dat,subset=Tr==0,offset=offset,...)

    yhatTfull <- predict(fullModT,newdata=dat,lambda=fullModT$lambda.min,type="response")
    yhatCfull <- predict(fullModC,newdata=dat,lambda=fullModT$lambda.min,type="response")


    that <- numeric(N)
    chat <- numeric(N)

### now fill in that and chat for each i. either use yhatXfull or the out of bag predictions
    that <- ifelse(Tr==1,[OOB],yhatTfull)
    chat <-
