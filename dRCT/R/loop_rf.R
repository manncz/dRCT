...
loop_rf2 = function(Y, Tr, Z, dropobs = NULL) {
  Y = as.matrix(Y)
  ...
  forest1 = randomForest::randomForest(Z[Tr==1,,drop=FALSE], Y[Tr==1,,drop=FALSE],
                                       sampsize=if(dropobs) length(Y[Tr==1])-1 else length(Y[Tr==1]))
  forest0 = randomForest::randomForest(Z[Tr==0,,drop=FALSE], Y[Tr==0,,drop=FALSE],
                                       sampsize=if(dropobs) length(Y[Tr==0])-1 else length(Y[Tr==0]))
  that[Tr==1] = predict(forest1)
  chat[Tr==0] = predict(forest0)

  out <- cbind(that,chat)

  attr(out,"forests") <- list(forest1=forest1,forest0=forest0)
...
loop_rf = function(Y, Tr, Z, dropobs = NULL) {
  ...
  forests <- list(forest1=forest1,forest0=forest0)
